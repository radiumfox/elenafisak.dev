...
export function FeatureCards({ items, action, className }: FeatureCardsProps) {
  return (
    <div className={`grid gap-px overflow-hidden rounded-2xl border border-line bg-line/60 sm:grid-cols-2 xl:grid-cols-3 ${className}`}>
      {items.map((item) => (
